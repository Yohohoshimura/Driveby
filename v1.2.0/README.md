...
1.2 is a correctness / hygiene release on top of 1.1. The sync mode introduced in 1.1 had a handful of latent bugs around concurrency, exclude-pattern semantics, FAT/exFAT mtime handling, and dead code left over from the 1.0 hardlink-incremental flow. 1.2 fixes them and surfaces the accent picker that was wired up in CSS but never reachable from the UI.
...
| 5 | **Restore points to a real path** *(documented)* | History entries continue to record `path = destination`, but see "Restore semantics" below — restore is documented as "this destination", not "this snapshot". The sync mode of 1.1+ doesn't write a manifest, so there is no separate snapshot folder to point at. |
