...
async fn execute(
    ...
    for file in &files {
        ...
        let mut skip = false;
        if !is_icon_descriptor(&file.rel) {
    ...
    let icon_files: Vec<&FileEntry> = files.iter()
    ...
    if !icon_files.is_empty() {
        ...
        for f in &icon_files {
            ...
            if Some(src_hash) != dst_hash {
                warn!("folder-icon descriptor {} differs from source — forcing re-copy", f.rel);
                if let Some(parent) = dest_path.parent() {
        ...
        if icon_resyncs > 0 {
            info!("re-synced {} folder-icon descriptor(s) after hash mismatch", icon_resyncs);
        } else {
            info!("verified {} folder-icon descriptor(s) — all match source", icon_files.len());
        }
    }
