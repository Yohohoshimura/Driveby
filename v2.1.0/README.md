...
2.1 builds on 2.0 with **UI motion** and a **Modify Task** flow. Previous releases (`v1.0.1-beta` … `v1.7.0-beta`, plus 2.0.0) remain in their own folders. The app was previously called **BackupDrive**.
...
All 1.7 features (hardlink incremental, restore, xxHash3 verify, retries, continue-on-error, mtime preservation, tracing logs, single-instance / window-state / updater plugins, UI state persistence) are retained.
