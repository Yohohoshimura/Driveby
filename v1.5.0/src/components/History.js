      ...
      <table className="history-table">
        <thead>
          <tr>
            ...
            <th>Task</th>
            <th>Status</th>
            <th className="history-table th--right">Size</th>
            <th className="history-table th--right">Files</th>
            <th className="history-table th--right">Duration</th>
            <th className="history-table th--right">Actions</th>
          </tr>
        </thead>
        <tbody>
          {filtered.map((entry) => (
            <tr key={entry.id}>
              ...
              <td className="history-table td--right"><span className="mono">{entry.totalBytes ? formatBytes(entry.totalBytes) : '—'}</span></td>
              <td className="history-table td--right"><span className="mono">{entry.totalFiles ?? '—'}</span></td>
              <td className="history-table td--right"><span className="mono">{entry.durationMs ? formatDuration(Math.round(entry.durationMs / 1000)) : '—'}</span></td>
              <td className="history-table td--right">
                {entry.path && (
