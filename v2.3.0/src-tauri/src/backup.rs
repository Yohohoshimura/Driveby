...
fn apply_attrs(p: &Path, attrs: u32) {
    ...
    if ok == 0 {
        ...
        warn!("SetFileAttributesW failed for {}: GetLastError={}", lp.display(), err);
    }
}
...
async fn execute(
    ...
    // Mirror directory attributes — this is the half of "custom folder icon"
    // that lives on the parent folder (Explorer needs both a `desktop.ini`
    // file *and* a Readonly or System bit on the parent to render it). The
    // file half is preserved by `copy_file`, this loop preserves the
    // directory half.
    //
    // Sort deepest-first so the System/Readonly bit on a child folder is
    // applied before any ancestor inherits a Readonly bit that would block
    // mutation, and ensure every source dir has a destination counterpart
    // (empty source subfolders aren't created by the file-copy loop).
    let mut dir_targets: Vec<(PathBuf, String)> = dirs.iter()
        .map(|(p, r)| (p.clone(), r.clone()))
        .collect();
    dir_targets.sort_by_key(|(_, r)| std::cmp::Reverse(r.len()));
    for (src_dir, rel) in &dir_targets {
